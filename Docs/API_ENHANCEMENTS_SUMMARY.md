...
**Purpose:** Remove/dismiss a barcode from the unresolved list
...
**Purpose:** Associate a barcode with a Grocy product
...
1. **[api/index.php](../api/index.php)**
   - Enhanced routing system to support path parameters (`{id}`)
   - Updated `BBuddyApi::execute()` to handle pattern matching
   - Updated `ApiRoute` class with pattern matching support
   - Updated `/system/unknownbarcodes` route (lines 192-212)
   - Added DELETE `/system/unknownbarcodes/{id}` route (lines 214-230)
   - Added POST `/system/unknownbarcodes/{id}/associate` route (lines 232-307)
   - Added GET `/system/barcodelogs` route (lines 309-325)

2. **[incl/db.inc.php](../incl/db.inc.php)**
   - Added new method `getLogsWithId(int $limit)` (lines 564-579)
...
- Constructor accepts optional HTTP method filter
...
1. **PATCH `/api/system/unknownbarcodes/{id}`**
   - Update barcode metadata (name, possibleMatch, etc.)

2. **GET `/api/system/unknownbarcodes/{id}`**
   - Fetch single barcode details

3. **POST `/api/system/unknownbarcodes/batch-associate`**
   - Associate multiple barcodes at once

4. **Timestamp fields**
   - Add `createdAt` to barcodes for aging analysis
   - Add `timestamp` to logs

5. **Pagination**
   - Add `offset` parameter to both endpoints
...
- ✅ PHP syntax validated (manual review)
